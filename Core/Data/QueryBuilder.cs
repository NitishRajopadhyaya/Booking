...
using EditableAttribute = Core.Data.CRUD.Attribute.EditableAttribute;
using KeyAttribute = System.ComponentModel.DataAnnotations.KeyAttribute;
using ReadOnlyAttribute = Core.Data.CRUD.Attribute.ReadOnlyAttribute;
...
namespace Core.Data
{
    public abstract class QueryBuilder
    {
        ...
        /// <summary>
        /// update status column as 'D' to mark as deleted deleted
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public QueryRequest DeleteWithStatusFlag<T>(int id)
        {
            var entity = typeof(T);
            string primaryKey = entity.GetProperties().FirstOrDefault(p => p.GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(KeyAttribute).Name)).Name;
            var currenttype = typeof(T);
            var name = GetTableName(currenttype);
            var sb = new StringBuilder();
            sb.AppendFormat("update {0}", name);
            sb.AppendFormat(" set Status='6' where {0}={1}  ", primaryKey, id);
            return new QueryRequest { QuerySql = sb.ToString() };
        }
